...
6. Сохраняет URL статей в память Hermes — повторный запрос той же темы не покажет уже виденные материалы

**Level 2:** аннотации + цельная обзорная статья (не просто список ссылок)  
**Level 3:** дедупликация через встроенную память Hermes (`MEMORY.md`)
